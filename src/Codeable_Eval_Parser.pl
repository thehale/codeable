...
term(term_times(T1, T2)) --> factor(T1), [*], term(T2).
term(term_divide(T1, T2)) --> factor(T1), [/], term(T2).

expr(expr_term(E)) --> term(E).
expr(expr_plus(E1, E2)) --> term(E1), [+], expr(E2).
expr(expr_minus(E1, E2)) --> term(E1), [-], expr(E2).
...
assignment(assign(I, E)) --> identifier(I), [:=], expr(E).
